...
Step-by-step deployment for both Uberspace generations. The installer (`ta install`) is version-agnostic — these notes explain what differs under the hood and what to watch for.
...
> **Tip:** If you set `MONGO_URI` in the LibreChat `.env` before running `ta install`, the installer auto-derives `MONGO_URI_SIGNALS` (same cluster, `signals` database).

---

## Step 5: Start Services

The `ta` command abstracts U7/U8 differences, but you can also use native commands:

**Using `ta` (recommended, works on both):**
```bash
ta restart
ta status
```
...
```bash
ta agents you@example.com yourpassword
```
...
4. Verify cron is set up:
   ```bash
   crontab -l | grep ta
   # */15 * * * * ~/bin/ta cron 2>&1 | logger -t ta-cron
   ```
...
**You don't need to remember these.** The `ta` command abstracts them:
```bash
ta status    # works on both
ta restart   # works on both
ta logs      # works on both
```
...
5. **Restore MongoDB** (if needed):
   ```bash
   ta restore path/to/backup.json.gz
   ```
