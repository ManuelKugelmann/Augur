...
async def unhcr_population(year: int = 2024, country_origin: str = "",
    ...
    return await api_get("https://api.unhcr.org/population/v1/population/",
                         params=params, label="UNHCR")
...
async def unhcr_demographics(year: int = 2024, country_asylum: str = "") -> dict:
    ...
    if country_asylum:
        params["coa"] = country_asylum
    return await api_get("https://api.unhcr.org/population/v1/demographics/",
                         params=params, label="UNHCR demographics")
...
async def hdx_search(query: str, rows: int = 20) -> dict:
    ...
    return await api_get("https://data.humdata.org/api/3/action/package_search",
                         params={"q": query, "rows": rows}, label="HDX search")
...
async def hdx_dataset(dataset_id: str) -> dict:
    ...
    return await api_get("https://data.humdata.org/api/3/action/package_show",
                         params={"id": dataset_id}, label="HDX dataset")


# ── ReliefWeb (shared filter builder + POST) ──

def _rw_filters(filters: list[dict]) -> dict | None:
    """Build ReliefWeb filter from list of field/value dicts."""
    if len(filters) == 1:
        return filters[0]
    elif len(filters) > 1:
...
async def reliefweb_reports(query: str = "", country: str = "",
    ...
    filt = _rw_filters(filters)
    if filt:
        body["filter"] = filt
    return await api_post("https://api.reliefweb.int/v2/reports",
                          json=body, params={"appname": RELIEFWEB_APPNAME},
                          headers=_RW_HEADERS, label="ReliefWeb reports")
...
async def reliefweb_disasters(country: str = "", status: str = "ongoing",
    ...
    if filt:
        body["filter"] = filt
    return await api_post("https://api.reliefweb.int/v2/disasters",
                          json=body, params={"appname": RELIEFWEB_APPNAME},
                          headers=_RW_HEADERS, label="ReliefWeb disasters")


# ── IDMC (shared pattern) ──

async def _idmc_query(endpoint: str, iso3: str, start_year: int,
                      end_year: int) -> dict:
    """Shared IDMC displacement query."""
    if not IDMC_KEY:
    ...
    return await api_get(
        f"https://helix-tools-api.idmcdb.org/external-api/gidd/{endpoint}",
        params=params, label=f"IDMC {endpoint.split('/')[-1]}")
...
async def idmc_displacement(iso3: str = "", start_year: int = 0,
    ...
    return await _idmc_query("displacements/displacement-export",
...
async def idmc_disasters(iso3: str = "", start_year: int = 0,
    ...
    Requires IDMC_API_KEY env var."""
    return await _idmc_query("disasters/disaster-export",
                             iso3, start_year, end_year)
...
async def humanitarian_crisis(country: str = "", iso3: str = "") -> dict:
    ...
    calls: dict = {}
    if iso3:
        calls["refugees"] = unhcr_population(country_asylum=iso3)
    name = country or iso3
    if name:
        calls["disasters"] = reliefweb_disasters(country=country, limit=10)
        calls["reports"] = reliefweb_reports(country=country, limit=10)
