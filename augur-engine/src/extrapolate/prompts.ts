import type { BrandConfig, HorizonKey, Locale } from '../config/types.js';
import { SECTION_LABELS } from '../config/horizons.js';

/** Build system prompt for Pass 1: signal → extrapolation */
export function systemPromptPass1(brand: BrandConfig): string {
  return `${brand.tonePrompt}

You produce structured predictions for ${brand.name}. Each prediction has three sections:

1. "${SECTION_LABELS[brand.locale].signal}" — What's actually happening right now. Factual, uncomfortable, sourced. Cite specific data points from the provided signals.

2. "${SECTION_LABELS[brand.locale].extrapolation}" — Where this leads if unchecked. A wake-up call, not sensationalism. Be specific about timeframes and consequences.

3. "${SECTION_LABELS[brand.locale].inTheWorks}" — Who's working on solutions. Real efforts, named, sourced — not hopium. If no credible solution exists, say so.

Rules:
- Never fabricate statistics or data points
- Cite the actual signals provided
- ${brand.locale === 'de' ? 'Write in German (Hochdeutsch)' : 'Write in English, AP/Reuters style'}
...
Output MUST be valid JSON matching this schema:
{
  ...
  "confidence": "high | medium | low"${brand.module === 'markets' ? `,
...
export function userPromptPass1(
  signals: unknown[],
  horizon: HorizonKey,
  fictiveDate: string,
  locale: Locale,
): string {
  const horizonLabel = locale === 'de'
    ? { tomorrow: 'Morgen', soon: 'Bald (1 Monat)', future: 'Zukunft (1 Jahr)' }[horizon]
    : { tomorrow: 'Tomorrow', soon: 'Soon (1 month)', future: 'Future (1 year)' }[horizon];

  return `Horizon: ${horizonLabel}
Fictive prediction date: ${fictiveDate}
...
export function systemPromptPass2(locale: Locale): string {
  const lang = locale === 'de' ? 'German' : 'English';
  return `You are an editor. You receive a prediction article as JSON. Your job:
...
export function systemPromptPass3(platforms: string[], locale: Locale): string {
  return `You generate social media captions for a prediction article. Output JSON:

{
  "captions": {
    ${platforms.map(p => `"${p}": "string — platform-native caption"`).join(',\n    ')}
  }
}
