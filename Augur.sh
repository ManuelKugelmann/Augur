...
SVCEOF
        ...
        # Bootstrap profile data via agent (if API key set)
        if [[ -n "${AUGUR_AGENTS_API_KEY:-}" ]]; then
            ...
            if [[ "$LC_READY" == true ]]; then
                log "Bootstrapping data (all phases)..."
...
SVCEOF
    ...
    bootstrap)
        ...
        if [[ "${2:-}" == "--dry-run" ]]; then
            ...
            echo ""
            echo "  Populates profile data via the bootstrap agent (all phases by default)."
            echo "  Reads AUGUR_AGENTS_API_KEY from .env, auto-discovers agent ID."
            echo "  Safe to re-run — deepens existing data each time."
            ...
            echo ""
            echo "  Phases: profiles → timeseries → events → plans"
        else
            echo -e "${CYAN}Bootstrapping via ${LC_URL} (all phases)...${NC}"
            "$STACK/venv/bin/python" "$STACK/augur-uberspace/scripts/bootstrap-data.py" \
                --base-url "$LC_URL" "${@:2}"
...
PYEOF
    ...
    *)
        ...
        echo "  augur bootstrap    Bootstrap data via agent (all phases, re-runnable)"
