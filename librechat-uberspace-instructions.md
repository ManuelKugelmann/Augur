...
GitHub repo ──tag──▶ CI builds frontend + bundles tar ──▶ GitHub Release
                            ...
                            Uberspace (runtime)
                            ├─ LibreChat (:3080)
                            ├─ MCP: filesystem  → ~/librechat-data/files/
                            ├─ MCP: memory      → ~/librechat-data/memory.jsonl
                            ├─ MCP: sqlite      → ~/librechat-data/data.db
...
This script:
- Clones/initializes `~/librechat-data/` as a git repo
...
```bash
crontab -l | grep librechat-data-sync
# Should show: */15 * * * * cd ~/librechat-data && git add -A && ...
```
...
|---|---|---|
| `~/librechat-data/files/` | `server-filesystem` | Documents, exports, CSV files |
| `~/librechat-data/memory.jsonl` | `server-memory` | Knowledge graph (entities + relations) |
| `~/librechat-data/data.db` | `mcp-sqlite` | SQLite database (logs, research, notes) |

View history: `cd ~/librechat-data && git log --oneline`
...
After install, `~/bin/lc` provides shortcuts:

```bash
lc s          # status + version
lc l          # tail live logs
lc r          # restart service
lc v          # installed version
lc u          # update to latest release
lc rb         # rollback to previous version
lc sync       # force git-sync data now
lc env        # edit .env
...
# Disk usage
du -sh ~/LibreChat ~/librechat-data
```
...
# Symlink it
ln -sf /path/found/memory.jsonl ~/librechat-data/memory.jsonl
```
