...
_pip_install() {
    ...
    log "  → $python -m pip install -v --only-binary numpy,pandas,pydantic_core,rpds-py,watchfiles,pymongo,caio --prefer-binary -r $req ${*:3}"
    "$python" -m pip install -v --only-binary numpy,pandas,pydantic_core,rpds-py,watchfiles,pymongo,caio --prefer-binary -r "$req" "${@:3}" </dev/null
}
