...
curl -fL --create-dirs -o /tmp/lc-install/bundle.tar.gz "$URL"
mkdir -p ~/LibreChat && tar xzf /tmp/lc-install/bundle.tar.gz -C ~/LibreChat && rm -rf /tmp/lc-install
...
mkdir -p ~/bin && cp ~/augur/Augur.sh ~/bin/augur && chmod +x ~/bin/augur
